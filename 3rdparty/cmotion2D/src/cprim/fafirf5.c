...
/*
 ...
 DESCRIPTION	: Le fichier (fafirf5 = Float Array Finite Impulse Response
		  Filter 5) contient les procedures de filtrage spatial de
 ...
 * filtre "fir" de 5 coefficients et stocke les elements filtres dans le
 * tableau de reels simple precision "dst".
 *
 * HISTORIQUE	:
 * 1.00 - 26/07/95 - Original.
 */
void	firf5_float (const float *src, float *dst, size_t n, const float *fir)
{
 ...
 */
void	firf5inc_float (const float *src, float *dst, size_t n,
 ...
 * DESCRIPTION	:
 * La procedure filtre le tableau de "n" reels simple precision "src" par le
 * filtre symetrique "fir" de 5 coefficients et stocke les elements filtres
 * dans le tableau de reels simple precision "dst".
 *
 * HISTORIQUE	:
 * 1.00 - 27/07/95 - Original.
 */
void	firf5sym_float (const float *src, float *dst, size_t n,
		const float *fir)
{
	const float	*send = src + n;	/* borne de "src"	*/

	for  (; src < send; src++, dst++)
		*dst = ((src[0] + src[4]) * fir[0])
		     + ((src[1] + src[3]) * fir[1])
		     + ((src[2])	  * fir[2]);
}

/*
 * PROCEDURE	: firf5syminc_float
 *
 * INPUT	:
 * src		Tableau de reels simple precision source.
 *
 ...
 * 1.00 - 27/07/95 - Original.
 */
void	firf5syminc_float (const float *src, float *dst, size_t n,
		int incsrc, int incdst, const float *fir)
{
	size_t	inc1 = incsrc * 1;	/* multiples de l'increment	*/
	size_t	inc2 = incsrc * 2;
	...
	for (i = 0; i < n; i++) {
		*dst = ((src[0]	   + src[inc4])	* fir[0])
		     + ((src[inc1] + src[inc3])	* fir[1])
		     + ((src[inc2])		* fir[2]);
		src += incsrc;
		dst += incdst;
	}
...
void	main (int argc, char *argv[])
{
	...
	LOOP(firf5_float (src, dst, BUFSIZE-5, fir), basen);
	LOOP(firf5inc_float (src, dst, BUFSIZE-5, 1, 1, fir), basen);
	LOOP(firf5sym_float (src, dst, BUFSIZE-5, fir), basen);
	LOOP(firf5syminc_float (_src, dst, BUFSIZE-5, -1, 1, fir), basen);
	LOOP(firf5syminc_float (_src, _dst, BUFSIZE-5, -1, -1, fir), basen);
