...
/*
 ...
 DESCRIPTION	: Le fichier (fafirf7 = Float Array Finite Impulse Response
		  Filter 7) contient les procedures de filtrage spatial de
		  7 coefficients operant sur les tableaux de reels simple
 ...
 * filtre "fir" de 7 coefficients et stocke les elements filtres dans le
 * tableau de reels simple precision "dst".
 *
 * HISTORIQUE	:
 * 1.00 - 26/07/95 - Original.
 */
void	firf7_float (const float *src, float *dst, size_t n, const float *fir)
{
 ...
 */
void	firf7inc_float (const float *src, float *dst, size_t n,
		int incsrc, int incdst, const float *fir)
{
	size_t	inc1 = incsrc * 1;	/* multiples de l'increment	*/
	size_t	inc2 = incsrc * 2;
	size_t	inc3 = incsrc * 3;
	size_t	inc4 = incsrc * 4;
	size_t	inc5 = incsrc * 5;
	size_t	inc6 = incsrc * 6;
 ...
 * DESCRIPTION	:
 * La procedure filtre le tableau de "n" reels simple precision "src" par le
 * filtre symetrique "fir" de 7 coefficients et stocke les elements filtres
 * dans le tableau de reels simple precision "dst".
 *
 * HISTORIQUE	:
 * 1.00 - 27/07/95 - Original.
 */
void	firf7sym_float (const float *src, float *dst, size_t n,
		const float *fir)
{
	const float	*send = src + n;	/* borne de "src"	*/

	for  (; src < send; src++, dst++)
		*dst = ((src[0] + src[6]) * fir[0])
		     + ((src[1] + src[5]) * fir[1])
		     + ((src[2] + src[4]) * fir[2])
		     + ((src[3])	  * fir[3]);
}
 ...
 * 1.00 - 27/07/95 - Original.
 */
void	firf7syminc_float (const float *src, float *dst, size_t n,
		int incsrc, int incdst, const float *fir)
{
	size_t	inc1 = incsrc * 1;	/* multiples de l'increment	*/
	size_t	inc2 = incsrc * 2;
	...
	for (i = 0; i < n; i++) {
		*dst = ((src[0]	   + src[inc6])	* fir[0])
		     + ((src[inc1] + src[inc5])	* fir[1])
		     + ((src[inc2] + src[inc4])	* fir[2])
		     + ((src[inc3])		* fir[3]);
		src += incsrc;
		dst += incdst;
	}
...
void	main (int argc, char *argv[])
{
	...
	LOOP(firf7_float (src, dst, BUFSIZE-7, fir), basen);
	LOOP(firf7inc_float (src, dst, BUFSIZE-7, 1, 1, fir), basen);
	LOOP(firf7sym_float (src, dst, BUFSIZE-7, fir), basen);
	LOOP(firf7syminc_float (_src, dst, BUFSIZE-7, -1, 1, fir), basen);
	LOOP(firf7syminc_float (_src, _dst, BUFSIZE-7, -1, -1, fir), basen);
